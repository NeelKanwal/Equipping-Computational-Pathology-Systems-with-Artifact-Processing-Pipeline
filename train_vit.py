...
# This file provides training code for vision transformer models mentioned in the paper.
...
if __name__ == "__main__":
    ...
    architectures = ["ViT"] 
    pretrained = True  # if false then train from scratch
    ...
    val_compose = transforms.Compose([transforms.Resize((224, 224)), transforms.ToTensor(),
                                      transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
            ...
            for sch in lr_scheduler:
                for lr in learning_rate:
                    for i in range(iterations):
                        ...
                        # roc_ovr = roc_auc_score(y_true, y_pred, multi_class='ovr')
                        # print("ROC AUC (OVR): ", roc_ovr)

                        # roc_ovo = roc_auc_score(y_true, y_pred, multi_class='ovo')
                        # print("ROC AUC (OVO): ", roc_ovo)
                        ...
                        # mac_precision = average_precision_score(y_true, y_pred, average='macro')
                        ...
                        # print("Average Precision: ", mac_precision)
                        ...
                        if test:
                            ...
                            y_pred, y_true, probs, _, _ = epoch_test_transformer(model, test_loader, criterion)
                            y_pred = convert_batch_list(y_pred)
                            y_true = convert_batch_list(y_true)
                            probs = convert_batch_list(probs)
